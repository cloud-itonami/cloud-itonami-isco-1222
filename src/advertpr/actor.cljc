(ns advertpr.actor
  "AdvertisingPRManagementActor — the ISCO-08 1222 community advertpr actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one advertpr operation request
  ...
  The unconditional invariant: the AdvertisingPRManagementAdvisor can never
  directly commit a record the AdvertisingPRManagementGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [advertpr.advisor :as advisor]
            [advertpr.governor :as governor]
            [advertpr.store :as store]))

(defn build-graph
  "Build a compiled AdvertisingPRManagementActor graph. `store` implements
  `advertpr.store/Store`. `advisor` implements
  `advertpr.advisor/Advisor` (defaults to `mock-advisor`).
